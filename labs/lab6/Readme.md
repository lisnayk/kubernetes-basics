...
6.  **Додаткове завдання: Реалізувати дефолтну сторінку 404 (Default Backend)**:
    - Підготувати маніфест Ingress, де окрім основних правил буде вказано `spec.defaultBackend`.
    - Створити окремий Deployment та Service (наприклад, `error-404`) зі спеціальним веб-інтерфейсом або використати один із існуючих сервісів (наприклад, `blog-service`) як дефолтний для всіх невідомих запитів.
    - Налаштувати `defaultBackend` у вашому Ingress-ресурсі так, щоб при зверненні до неіснуючого шляху (наприклад, `http://localhost/anything`) або невідомого хоста відкривалася обрана дефолтна сторінка.
    - Виконати перевірку, переконавшись, що Ingress Controller більше не повертає стандартну сторінку "404 Not Found" від Nginx.
